#Server version of web scraping file.
...
def fetch_medal_data(url):
    ...
    chrome_options.add_argument("--disable-gpu")  # GPU hardware acceleration isn't necessary for headless mode
    ...
    try:
        ...
        for data in medal_data:
            ...
            if not all(key in data for key in ['ranking', 'country', 'gold', 'silver', 'bronze', 'overall']):
                raise ValueError(
                    "Each data item must contain all keys: ranking, country, gold, silver, bronze, overall")
            ...
            cursor.execute("""
                ...
                """,
                           (data['ranking'], data['country'], data['gold'], data['silver'], data['bronze'],
                            data['overall'])
                           )
